			...
			if(empty($page) || is_null($page)){
				if(!$is_logged_in) {
					...
					if(is_numeric($this->Author->get('default_area'))) {
						$section_handle = Symphony::Database()->fetchVar('handle', 0, "SELECT `handle` FROM 
`tbl_sections` WHERE `id` = '".$this->Author->get('default_area')."' LIMIT 1");

						if(!$section_handle){
							$section_handle = Symphony::Database()->fetchVar('handle', 0, "SELECT `handle` 
FROM `tbl_sections` ORDER BY `sortorder` LIMIT 1");
						}
...
$this->Author->get('default_area'));
			...
			include_once((isset($this->_callback['driverlocation']) ? $this->_callback['driverlocation'] : CONTENT) . 
'/content.' . $this->_callback['driver'] . '.php');
			...
			if(!$is_logged_in && $this->_callback['driver'] != 'login'){
				...
				if(file_exists(DOCROOT . '/update.php') && $this->__canAccessAlerts()) {
					if(file_exists(DOCROOT . '/README.markdown') && is_readable(DOCROOT . '/README.markdown')) {
						$readme = file(DOCROOT . '/README.markdown', FILE_IGNORE_NEW_LINES | 
FILE_SKIP_EMPTY_LINES);
						...
						if(version_compare($current_version, $readme, '<')) {
							$message = __('Run the updater to update Symphony to %s. <a href="%s">View 
Update</a>', array($readme, URL . "/update.php"));
						}
						...
						// The updater is the same version as the current Symphony install.
							...
							$message = __('Your Symphony installation is up to date, but an updater script was 
still detected. For security reasons, it should be removed. <a href="%s/update.php?action=remove">Remove Update Script</a>', array(URL));
						}
					}
					// Can't detect update Symphony version
					else {
						$message = __('An updater script has been found in your installation. <a href="%s">View 
Update</a>', array(URL . "/update.php"));
					}
				...
				if(is_array($extensions) && !empty($extensions) && $this->__canAccessAlerts()) {
					foreach($extensions as $handle => $about) {
						if($about['status'] == EXTENSION_REQUIRES_UPDATE) {
							$this->Page->pageAlert(
								__('An extension requires updating. <a href="%s">View Extensions</a>', 
array(SYMPHONY_URL . '/system/extensions/'))
							);
							break;
						}
					}
		...
		private function __canAccessAlerts() {
			if($this->Page instanceof AdministrationPage && $this->isLoggedIn() && 
Administration::instance()->Author->isDeveloper()) {
				return true;
			}
			else {
				return false;
		...
		public function errorPageNotFound(){
			$this->customError(__('Page Not Found'), __('The page you requested does not exist.'), 'error', array('header' => 
